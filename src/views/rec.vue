...
<script>
  ...
  export default {
    ...
    methods : {
      getOrgs : function () {
        this.$http.get('/api/rec/orgs').then(response => {
      ...
      getActiveOrgPulls: function (active_org, page) {
        ...
        this.$http.get('/api/rec/'+active_org+'/page/'+page).then(response => {
            this.pulls = []
      ...
      getTotalPulls: function (active_org) {
        this.$http.get('/api/rec/'+active_org+'/total_nums').then(response => {
